...
| `src/backend/types.ts` | Backend seams (`BackendSeams`) and the state-first `RuntimeBackend` port. |
| `src/backend/v1.ts` | `V1Backend`: v1 hooks (messages.transform, system.transform, autocontinue, compact_context) onto the port. |
| `src/backend/v2.ts` | `V2Backend`: v2 events (session.hook, event.subscribe, catalog.transform, tool.transform, session.prompt) onto the port. |

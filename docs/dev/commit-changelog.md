...
**manual** — the release workflow does NOT write or commit CHANGELOG.md.
The GitHub release body and the Discord embed read the newest `##`-headed
section of your hand-maintained CHANGELOG.md. This is the topmost section
in the file. Version bumping and tag creation still happen automatically.
...
The overrides apply only in `auto` mode. In `manual` mode the preview posts
the next version and the bump table only. It ignores both overrides.

## Generated vs manual

| Aspect | auto | manual |
|--------|------|--------|
| CHANGELOG.md | generated and committed by the release | written by hand |
| GitHub release body | generated changelog | topmost `##` section of CHANGELOG.md |
| Discord embed | generated changelog | topmost `##` section of CHANGELOG.md |
| PR preview | changelog + version + bump table | version + bump table |

...
## Status
Superseded by [ADR-004](ADR-004-v2-only-rewrite.md) (2026-09-25). The dual-runtime
export and the `src/backend/` seam are gone; the plugin targets OpenCode V2 only.
The `ctx.session.compact` and toast findings below still hold on 2.0.16.
